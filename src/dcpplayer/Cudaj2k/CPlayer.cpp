	...
	
{
	...
	NumBlock = 0;
	out = out_swap = NULL;
	win_w=win_h=0; mywin = NULL; Renderer = NULL;
...
Result_t CPlayer::InitialisationReaders(CDcpParse &DcpParse, bool FirstTime, CReel *ptrReel_i)
{
	...
	MxfFiles.resize(4);
	ptrReel = ptrReel_i;
        if (DcpParse.VideoOk && ptrReel!=NULL && ptrReel->ptrMainPicture) MxfFiles[0] = MyPath + "/" + ptrReel->ptrMainPicture->sPath;
        if (DcpParse.SoundOk && ptrReel != NULL && ptrReel->ptrMainSound) MxfFiles[1] = MyPath + "/" + ptrReel->ptrMainSound->sPath;
        if (DcpParse.SubOk && ptrReel != NULL && ptrReel->ptrSubtitle)
	{
